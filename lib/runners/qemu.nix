...
let
  ...
  inherit (microvmConfig) fqdn vcpu mem balloon initialBalloonMem deflateOnOOM hotplugMem hotpluggedMem user interfaces shares socket forwardPorts devices vsock graphics storeOnDisk kernel initrdPath storeDisk credentialFiles;
...
''
{
  ...
  then throw "qemu does not support hotpluggedMem"
    ...
    ) "${fqdn}: forwardPortsOptions only running with user network" (
